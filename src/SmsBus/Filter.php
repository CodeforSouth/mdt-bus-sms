...
class Filter 
{
    ...
    {
        $this->dateFilter = new PregReplace(array('pattern' => '/[^0-9\-\/]/', 'replace' => ''));
        $this->bodyFilter = new PregReplace(array('pattern' => '/[^a-z0-9_-\s\&\,]/', 'replace' => ''));
        $this->phoneFilter = new PregReplace(array('pattern' => '/[^0-9\+\-]/', 'replace' => ''));
    ...
    public function sanitizePost(array $data)
    {
        ...
        $sanitized['FromSate'] = isset($data['FromState']) ? substr($this->alphaFilter->filter($data['FromState']), 0, 4) : null;
        ...
        $sanitized['ToSate'] = isset($data['ToSate']) ? substr($this->alphaFilter->filter($data['ToSate']), 0, 4) : null;
